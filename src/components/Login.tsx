  ...
  return (
    <div className="relative max-w-sm w-full mx-auto rounded-3xl overflow-hidden ember-card animate-fade-in">
      ...
      <div className="bg-gradient-to-br from-flame-dark via-flame-smoke to-flame-dark px-6 pt-6 pb-8 relative overflow-hidden">
        {/* Animated flame glow */}
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-32 h-20 bg-flame-core/30 blur-3xl"></div>
        ...
        <div className="flex flex-col items-center gap-3 relative z-10">
          <div className="w-16 h-16 rounded-2xl glass-flame flex items-center justify-center animate-ember-glow">
            <Flame className="h-8 w-8 text-flame-hot animate-flame-flicker" />
          </div>
          <p className="text-flame-light text-xl font-flame font-semibold text-center">Welcome Back!</p>
          <p className="text-flame-light/60 text-sm">Sign in to continue trading</p>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-b from-flame-smoke to-flame-dark px-6 pt-6 pb-8 space-y-5">
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex items-center justify-between text-xs">
            <label className="flex items-center gap-2 text-flame-light/70 cursor-pointer">
              <input
                type="checkbox"
                checked={rememberMe}
                onChange={(e) => setRememberMe(e.target.checked)}
                className="w-4 h-4 rounded border-flame-core/30 bg-flame-dark text-flame-core focus:ring-flame-core/30"
              />
              Remember me
            </label>
            <button
              type="button"
              className="text-flame-hot font-medium hover:text-flame-glow transition-colors"
            >
              Forgot Password?
            </button>
          </div>
